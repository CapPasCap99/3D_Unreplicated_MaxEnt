...
int main() {
    ...
    input_contacts.open("/home/capucine/Documents/test/Data/Initial_Contacts/t_averaged_normalized_array.txt");
    ...
    for (int n =0; n<update_steps;n++) { //do iterative update scheme
        ...
        intermediate_energies.open ("/home/capucine/Documents/test/Data/Intermediate_Energies/energies_intermediate_" + std::to_string(n) + ".txt");
        ...
        intermediate_contacts.open ("/home/capucine/Documents/test/Data/Intermediate_Contacts/contacts_intermediate_" + std::to_string(n) + ".txt");
        for(int i = 0; i < pol_length; i++){ //output contact frequencies
            for(int j = 0; j < pol_length; j++){
    ...
    while (n_capture <number_of_threads) {
        ...
        //output final configurations for each thread
    int n_capture1 = 0;
    while (n_capture1 <number_of_threads) {
        for (int thread_num = 0; thread_num < number_of_threads; thread_num++) {
            if (polymer[thread_num][0][2] < 20) {
                std::ofstream final_configuration;
                final_configuration.open(
                        "/home/capucine/Documents/test/Data/Final_Configurations/configuration_init_2" +
                        std::to_string(n_capture) + ".txt");
                for (int i = 0; i < pol_length; i++) { //output contact frequencies
                    for (int j = 0; j < 3; j++) {
                        final_configuration << polymer[thread_num][i][j] << std::endl;
                ...
                final_configuration.close();
                std::ofstream z_position_final_configuration;
                z_position_final_configuration.open(
                        "/home/capucine/Documents/test/Data/Final_Configurations/z_position_final_configuration_" +
                        std::to_string(n_capture) + ".txt");
                for (int i = 0; i < pol_length; i++) { //output contact frequencies
                    z_position_final_configuration << polymer[thread_num][i][3] << std::endl;
                }
                z_position_final_configuration.close();
                n_capture1++;
            }
        }
    }
...
const int pol_length = 1620;//AAAH 405
...
int main() {
    ...
    // Read in starting interaction energies
    std::ifstream couplings;
    couplings.open("/Users/abeardedbearcap99/Documents/PHD/Hi-C data manipulation/data/energies_ccrescentus_wt_rep1.txt");
    //couplings.open ("/media/joris/raid_data/Chromosome_Maxent/Saved_energies/energies_smallcell.txt");
    for (int i = 0; i < pol_length; i++) { //read in energies
        for (int j = 0; j < pol_length; j++) {
            couplings >> Interaction_E[i][j];
    ...
    std::vector<std::vector<double> > reference_contacts(pol_length, std::vector<double>(pol_length));
    std::ifstream input_contacts;
    input_contacts.open("/Users/abeardedbearcap99/Documents/PHD/Hi-C data manipulation/data/t_averaged_normalized_array.txt");
    for (int i = 0; i < pol_length / 4; i++) { //read in contacts
        for (int j = 0; j < pol_length / 4; j++) {
            input_contacts >> reference_contacts[4 * i][4 * j];
            if (reference_contacts[4*i][4*j] == 0 && i != j && i != (j + 1) % (pol_length / 2) &&
    ...
    for (int n =0; n<update_steps;n++) { //do iterative update scheme
        ...
        //output intermediate results
        std::ofstream intermediate_energies;
        intermediate_energies.open ("/Users/abeardedbearcap99/Documents/PHD/Start_C++/MaxEnt-Chromosome-Caulobacter/Results/Energies_Intermediate/energies_intermediate_" + std::to_string(n) + ".txt");
        for(int i = 0; i < pol_length; i++){ //output contact frequencies
            for(int j = 0; j < pol_length; j++){
                intermediate_energies << Interaction_E[i][j] << std::endl;
            }
        ...
        std::ofstream intermediate_contacts;
        intermediate_contacts.open ("/Users/abeardedbearcap99/Documents/PHD/Start_C++/MaxEnt-Chromosome-Caulobacter/Results/Contacts_Intermediate/contacts_intermediate_" + std::to_string(n) + ".txt");
        for(int i = 0; i < pol_length; i++){ //output contact frequencies
            for(int j = 0; j < pol_length; j++){
                double contact = double(final_contacts[i][j]);
                intermediate_contacts << contact << std::endl;
    ...
    while (n_capture <number_of_threads) {
        for (int thread_num = 0; thread_num < number_of_threads; thread_num++) {
            if (polymer[thread_num][0][2] < 20) {
                std::ofstream final_configuration;
                final_configuration.open(
                        "/Users/abeardedbearcap99/Documents/PHD/Start_C++/MaxEnt-Chromosome-Caulobacter/Results/Final_Configurations/configuration_init_" +
                        std::to_string(n_capture) + ".txt");
                for (int i = 0; i < pol_length; i++) { //output contact frequencies
                    for (int j = 0; j < 3; j++) {
                        final_configuration << polymer[thread_num][i][j] << std::endl;
                ...
                std::ofstream z_position_final_configuration;
                z_position_final_configuration.open(
                        "/Users/abeardedbearcap99/Documents/PHD/Start_C++/MaxEnt-Chromosome-Caulobacter/Results/Final_Configurations/z_position_final_configuration_" +
                        std::to_string(n_capture) + ".txt");
                for (int i = 0; i < pol_length; i++) { //output contact frequencies
                    z_position_final_configuration << polymer[thread_num][i][j] << std::endl;
                }
                z_position_final_configuration.close();
                n_capture++;
            }
